...
# ============================================================
# MUStARD++ Qwen3-Omni Teacher N=8 Sampling
#
# Dataset:
#   MUStARD++ (English)
#
...
#   tensor parallel = 1
#
# Candidate pool will later be shared by:
#   - Best-of-8
#   - Diverse-8
# ============================================================
...
# Conservative starting point for one 96GB H100.
MAX_NUM_SEQS=8
...
# Keep this consistent with MCSD.
...
DATA_ROOT="data/mustard/processed"

RESULT_ROOT="results/mustard/teacher/sample_n8"
...
# IMPORTANT:
#
# ms-swift 4.2.2 currently calculates:
#
#   total_iters = dataset_len // batch_size
#
# Therefore each split must be exactly divisible by its
# sampling batch size, otherwise trailing examples are lost.
# ============================================================
...
show_progress() {
    ...
    # ETA
    #
    # Model loading time is deliberately excluded from the
    # throughput estimate. ETA begins after actual sampling
...
if [ ! -f "$WRAPPER" ]; then
    ...
    echo "ERROR: sampling wrapper not found:"
    echo
    echo "  $WRAPPER"
...
run_split() {
    ...
    if [ $(( num_inputs % batch_size )) -ne 0 ]; then
        ...
        echo "ERROR:"
        echo
        echo "Split size is not divisible by sampling batch size."
        echo
        echo "split:      $split"
    ...
    echo
    echo "============================================================"
    echo "MUStARD++ Qwen3-Omni N=8 Sampling"
    echo "============================================================"
    echo
    echo "Split:                 $split"
    echo "Input:                 $data"
    echo
    echo "Input samples:         $num_inputs"
    ...
    echo "Max vLLM sequences:    $MAX_NUM_SEQS"
    echo
    echo "Output:"
    echo "  $output_path"
    echo
    ...
    if [ -f "$output_path" ]; then
        ...
        if [ "$current_lines" -eq "$expected_lines" ]; then
            ...
            echo "ERROR:"
            echo
            echo "A final output file already exists,"
            echo "but its number of trajectories is unexpected."
            echo
            echo "Current:  $current_lines"
            echo "Expected: $expected_lines"
            echo
            echo "The file will NOT be overwritten automatically."
    ...
    echo
    echo "Loading Qwen3-Omni / initializing vLLM..."
    echo "ETA will appear after actual generation starts."
    echo
    ...
    while kill -0 "$sampling_pid" 2>/dev/null; do
        ...
        # ----------------------------------------------------
        # Begin throughput measurement only after a new batch
    ...
    if [ "$status" -ne 0 ]; then
        ...
        echo "  $status"
        echo
        echo "Existing progress has been preserved."
        echo
    ...
    if [ ! -f "$output_path" ]; then
        ...
        echo "but the final output file is missing:"
        echo
        echo "  $output_path"
    ...
    if [ "$actual_lines" -ne "$expected_lines" ]; then
        ...
        echo
        echo "Do not use this split for downstream processing yet."
...
echo
echo "============================================================"
echo "ALL MUStARD++ N=8 SAMPLING COMPLETE"
echo "============================================================"
echo
...
echo "Expected:"
echo
echo "  valid:  180 x 8 = 1440"
echo "  test:   181 x 8 = 1448"
echo "  train:  841 x 8 = 6728"
echo "  -------------------------"
echo "  total:           9616"
echo

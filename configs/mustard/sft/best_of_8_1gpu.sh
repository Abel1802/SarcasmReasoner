...
#
# Selection:
#   Among structurally valid and correct trajectories,
#   select the trajectory with the highest mean token logprob
...
#   Greedy-SFT for a controlled comparison.
# ============================================================
...
# Disable Qwen2.5-Omni speech-generation components.
...
# Avoid unnecessary tokenizer multiprocessing warnings.
export TOKENIZERS_PARALLELISM=false

# Flush logs immediately on HPC.
...
EXPECTED_TRAIN_SIZE=757
EXPECTED_VALID_SIZE=163

if [ "$TRAIN_SIZE" -ne "$EXPECTED_TRAIN_SIZE" ]; then
    echo "ERROR: unexpected Best-of-8 training size."
...
if [ "$VALID_SIZE" -ne "$EXPECTED_VALID_SIZE" ]; then
    echo "ERROR: unexpected Best-of-8 validation size."
...
# Notes:
#
# - The training recipe is identical to Greedy-SFT.
#
# - LoRA is applied to all linear layers in the language model.
#
# - Vision/audio towers and multimodal aligners are frozen.
#
# - The teacher <think> content has already been removed during
#   SFT data construction.
#
# - Each source contributes at most one trajectory:
#   the highest-mean-logprob trajectory among the valid,
#   correct N=8 teacher generations.
#
# - Only the structured assistant response contributes to SFT
#   loss under ms-swift's default loss configuration.
#
# - max_length=16384 matches the multimodal context length used
#   during teacher trajectory generation.
#
...
echo
echo "============================================================"
echo "MUStARD++ Best-of-8 SFT finished"
echo "============================================================"
